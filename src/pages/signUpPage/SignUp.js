...
export default function SignUp(props) {
  ...
  const validateInputs = () => { //authenticating input
    ...
    if (!email.value || !/\S+@\S+\.\S+/.test(email.value)) { //autenticating email
      setEmailError(true);
      setEmailErrorMessage('Please, LockIn to a Valid Email Address.');
      isValid = false;
    } else {
      setEmailError(false);
    ...
    if (!password.value || password.value.length < 6) { //authenticating password
      setPasswordError(true);
      setPasswordErrorMessage('Password must be at least 6 characters long.');
      isValid = false;
    } else {
      setPasswordError(false);
      setPasswordErrorMessage('LockedIn');
    }

    if (!name.value || name.value.length < 1) { //authenticating name
      setNameError(true);
  ...
  const handleSubmit = (event) => {//handling submit and storing it in local storage
    ...
    // Also store under legacy key for compatibility
    localStorage.setItem('sozo_user', JSON.stringify({ name, email }));
    navigate('/dashboard');
  };
        ...
        <img src={icon} alt ='company logo' width={'10%'}  />
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Button
              fullWidth
              variant="outlined"
              onClick={() => alert('Sign up with Google')} //will use firebase auth later
            ...
            <Button
              fullWidth
              variant="outlined"
              onClick={() => alert('Sign up with Facebook')} // will use firebase auth later
